...
import { useAddApproachMutation } from "../../../Redux/approach/approachApi";

export default function AddApproach() {
  const navigate = useNavigate();
  const [image, setImage] = useState([]);

  const [addApproach, { isLoading }] = useAddApproachMutation();
  ...
  const handleAddProject = async (e) => {
    ...
    try {
      const res = await addApproach(formData);
      if (res?.data?.success) {
        Swal.fire("", "Approach added successfully", "success");
        e.target.reset();
        setImage([]);
