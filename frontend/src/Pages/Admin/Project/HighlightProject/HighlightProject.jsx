...
export default function HighlightProject() {
  ...
  const [createHighlightProject, { isLoading: isCreating }] =
    useCreateHighlightProjectMutation();
  const [updateHighlightProject, { isLoading: isUpdating }] =
    useUpdateHighlightProjectMutation();
  ...
  useEffect(() => {
    if (projectData?.data) {
      ...
      setImage([
        {
          data_url: `${import.meta.env.VITE_BACKEND_URL}${projectData.data.image}`,
        },
      ]);
    }
  }, [projectData]);
  ...
  const handleProjectSubmit = async (e) => {
    ...
    try {
      let response;
      if (projectId) {
        response = await updateHighlightProject({
          id: projectId,
          formData,
        }).unwrap();
      } else {
